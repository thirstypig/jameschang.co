...
<!-- now-tldr -->
The personal site at jameschang.co — homepage, project deep-dives, and a Derek Sivers-style /now page that auto-updates from eight cron-synced feeds (WHOOP, Spotify, Plex, MLB stats, fantasy standings, Google Calendar, and two Goodreads shelves — currently-reading + read) plus 13 per-project activity cards synced daily from editorial config plus live GitHub events (the old fetch of each source repo's CLAUDE.md was removed 2026-06-04). **Recently shipped** (2026-08-05): a `pin` override so a maintenance-mode project can be held in back-burner regardless of commit recency (first user: one grouped card for five family sites), a `repo_key_mismatch()` detector for the GitHub rename redirect that silently orphaned a project's events, a CI fix — the workflow had been running 157 of 525 tests for four months — and a security fix for the copy layer leaking suppressed text through its heartbeat note. Earlier: a public bucket list, a feed staleness monitor that opens a GitHub issue when anything goes quiet, Google Calendar integration, and the cron-driven /now project hub. Plain HTML/CSS/JS on GitHub Pages — no build step, ~78 KB page weight, Lighthouse 100/100/100/100.
<!-- /now-tldr -->
...
**`pin` override (added 2026-08-05).** The recency rule answers *"was this touched recently?"* — but the section headings claim something stronger, *"am I still building this?"* Those diverge for a maintenance-mode project that gets frequent content edits. An optional `"pin": "active" | "backburner"` in `bin/projects-config.json` skips the recency rule for that slug and forces its section. First and only user today: `family-sites`, which would otherwise be promoted to active every time a family site's copy is edited. **Do not fake this by emptying `shipping_repos[]`** — that also kills the card's "shipped" line and lies about the data; the pin keeps the activity box live while overriding only the section. An unrecognized pin value (`"back-burner"`, a typo) is logged to the cron log and falls through to recency rather than crashing; `TestLoadConfig::test_config_pin_values_are_all_recognized` catches it in CI, where it's loud. Constants `PIN_ACTIVE` / `PIN_BACKBURNER` / `VALID_PINS` live in `bin/update-projects.py`. The map is built by `pins_from_config()`, which keys on **presence** of the `pin` key rather than truthiness — an inline `if p.get("pin")` would silently drop `"pin": ""` and exempt exactly the likely typos from the fail-loudly contract. **A pinned back-burner project also sorts to the BOTTOM of its section** via `pin_backburner_last()`: recency ranking is meaningless for a project pinned precisely because activity doesn't describe it, and without this the most frequently pushed pinned project wins the section's most prominent card. **Call order matters** — `pin_backburner_last()` must run before `pin_self_last()` so jameschang-co keeps the absolute last slot; there is a test pinning that. Tests: `tests/test_projects.py::TestProjectPinning`, `TestPinsFromConfig`, `TestPinBackburnerLast`, `TestConfigPinsReachTheClassifier`.

**Housekeeping rules — what does NOT count as shipping (added 2026-09-01, `todos/148`).** The recency rule counts any Push/PR/Release, so one chore mirrored across every repo used to promote four or five projects at once. Two named, tunable rules in `bin/update-projects.py`, both applied inside `parse_events()` so `most_recent_event_time()` and `events_for_project()` read the same filtered dict and **cannot disagree** (a card reading back-burner while displaying a recent commit was the trap). **`BOT_ACTORS`** (`github-actions[bot]`) — `actor.login` is already in the events payload, so it costs no fetch and cannot misfire; bots are dropped BEFORE enrichment so the `MAX_COMMIT_ENRICHMENTS` budget goes to real commits. It affects exactly **one** repo: `jameschang.co` is the only one a bot pushes to (29 of its last 30 events), a no-op everywhere else. **`CHORE_PATTERNS`** (`port[\s\-_]*registry`) — deliberately narrow; a `chore:` prefix filter provably drops real work (see `todos/148`'s table). The separator class is load-bearing: the chore appears as "port registry" in a subject and "port-registry" in a merge branch name. Enrichment is lazy, so extra fetches happen only when a chore is actually in front. **`jameschang-co` carries `"pin": "active"`** as a direct consequence — filtering bots leaves its feed 29/30 empty with human pushes buried past the API window, so recency stops describing it. **Known limit:** a push is judged by its HEAD commit alone (`payload.commits[]` is empty from the events API even for public repos), so a push that shipped real work and ended with a chore reads as a chore; accepted because the fan-out this targets arrives as its own single-commit push. Tests: `tests/test_projects.py::TestHousekeepingRules`, `::TestSelfRepoPin`.

**Card format** (`.nb-proj-card`, added 2026-06-04): activity-first layout — the most recent shipped commit appears BEFORE the description so returning visitors see the delta without scanning prose. Each card has: (1) project name + domain, (2) status badge (`<span class="nb-proj-badge nb-proj-badge--{status}">`) with an inline Tabler SVG icon + "Status · Maturity" label (e.g. "Shipping · Beta", "Live · Private", "Blocked · Alpha"), (3) `.nb-proj-activity` inset with the most recent GitHub event link + live-relative timestamp (or `.nb-proj-activity--empty` if no recent events), (4) one-sentence description, (5) an optional `.nb-proj-links` row for a card standing in for several live sites, (6) "next up" line. Badge CSS tokens: `--color-border-info` (blue/shipping), `--color-border-success` (green/live), `--color-border-warning` (amber/blocked) — all defined in `notebook.css` with light/dark variants. Active section uses `.nb-grid-1` (full-width); back-burner uses `.nb-grid-3` (3-column responsive). Grid containers carry `id="proj-active"` and `id="proj-backburner"` for the opt-in JS layer.

**Config** at `bin/projects-config.json` — `{slug, repo, name, url, url_label, status_badge, maturity, desc, next_up, roadmap_items[], shipping_repos[], pin?, links?[]}` per project. `links[]` (`{label, url}` pairs) is optional and exists for a card that stands in for **more than one** live site — the name link handles the single-product case, so without it a grouped card can only name its domains as unclickable prose. Entries whose URL fails `safe_url()` are dropped rather than rendered dead. Rendered as `.nb-proj-links` after the description, reusing `.nb-proj-domain`'s mono/11px/accent tokens. `status_badge` is a plain word (`shipping` / `live` / `blocked` / `shipped`). Icons: shipping → code, live+public → globe, live+private → lock, blocked → clock, **shipped → check** (added 2026-08-05; `shipped` previously fell through to the code glyph meaning "actively shipping", one letter from its opposite). `shipped` deliberately has **no** `.nb-proj-badge--shipped` CSS modifier — notebook.css comments the base rule as "base (neutral/shipped)", so it falls through to neutral on purpose. `maturity` is `alpha` / `beta` / `public` / `private`. `desc`, `next_up`, and `roadmap_items` are editorial prose written in config — **the script no longer fetches CLAUDE.md from source repos** for the /now cards (that fetch was removed 2026-06-04; `extract_tldr()` and `_render_markdown_inline()` remain in the file as dead code for test backward-compatibility). **13 projects** currently configured: aleph, fantastic-leagues, bahtzang-trader, judge-tool, tabledrop, tastemakers, thirsty-pig, ktv-singer, jameschang-co (self-classifying), family-sites (one grouped card for five personal sites built for family — shengchangmd, minmeychang, jarrenchang, tobinchang, rhyschang; added 2026-08-05, `pin: "backburner"`, blank `url`/`url_label` so the name renders as `href="#"`), vouch (trust-first student-mentor platform; added 2026-07-01, no live URL yet so `url`/`url_label` are blank — back-burner until the private `thirstypig/vouch` repo emits a Push/PR/Release event), tip (voice app for rehearsing a dreaded conversation against an AI counterpart, then getting scored on how you steered it; added 2026-07-10 as **spar**, renamed to **TIP** 2026-08-04 when the source repo became `thirstypig/TIP` — private, deployed at `tip.bahtzang.com`), pasadenaworks (Astro marketing site for a small-business consultancy in Pasadena; added 2026-09-01, live and public at `pasadenaworks.com`, so `live` + `public` → globe icon. Its `shipping_repos[]` carries a SECOND repo, the private `thirstypig/property-page`, whose password-gated pages are served from `pasadenaworks.com` subdomains — the card folds that work in as activity while publishing nothing about it. **Deliberately no `links[]` and no mention in `desc`:** that repo's README holds a live gate password, a Cloudflare account + KV namespace ID, and identifiable property addresses, and `bin/projects-config.json` is committed to this PUBLIC repo, so anything written here publishes twice — config and rendered HTML. `TLDR_FETCH_TOKEN` was granted `property-page` on 2026-09-01 (21 repos, Contents+Metadata read-only). **STANDING CONSTRAINT — commit messages in `thirstypig/property-page` are PUBLISHABLE.** `parse_events()` enriches the newest push per repo by fetching `/repos/{repo}/commits/{sha}` **with the PAT** and rendering the message's first line as the card's link text on the public /now page. GitHub strips private-repo payloads from the events endpoint; this enrichment fetches them back. So whenever property-page's newest push is more recent than pasadenaworks', its commit subject and a link naming the private repo appear on jameschang.co. This is pre-existing behaviour for every private card (`alephco.io-app`, `thejudgetool` both publish subjects today) — **accepted deliberately on 2026-09-01** after weighing removal (drop the repo from `shipping_repos[]`) and per-repo enrichment suppression. The cost of the choice is that it is enforced by memory, not by code: **write property-page commit subjects as if they are public, because they are.** No addresses, no gate passwords, no client names in a commit subject line. Nothing else about that repo is published — no `links[]`, no mention in `desc`.) **Renaming a source repo breaks classification (now detected, 2026-08-05):** GitHub redirects `/repos/{old}/events` to the new name, so the fetch still returns 200, but the payload stamps `repo.name` with the NEW name while `most_recent_event_time()` looks events up by the config string — a key miss reads as "no events" and the project falls to back-burner. When a repo is renamed, update `repo` + `shipping_repos[]` here in the same change. `repo_key_mismatch()` now compares the configured string against the name GitHub actually filed the events under and, on any difference, logs a warning and records it via `mismatch_heartbeat_kwargs()` → `record_heartbeat(partial_success=True)`, so it surfaces in `.feeds-heartbeat.json` as `last_error` without false-tripping the 48h monitor. Two deliberate choices: the comparison is **exact** (the invariant is "does the config string match the key `parse_events` files under", so a case-only drift is reported too), and the mismatch is **report-only, never self-healed** — re-keying to the returned name would keep the card correct and thereby remove the only pressure to fix the config. Blind spot: a repo with zero events carries no name to compare, so a rename on a genuinely quiet repo is undetectable — acceptable, since back-burner is the right answer for it anyway. Tests: `tests/test_projects.py::TestRepoKeyMismatch`. **Required GitHub Secret:** `TLDR_FETCH_TOKEN` (fine-grained PAT with read access to every private `shipping_repos[]` entry — needed for per-repo events, not CLAUDE.md). **Events are fetched per-repo** via `/repos/{repo}/events`; a single repo's failure is isolated (`[]`), never fatal. Fail-safe: missing markers → heartbeat error + bail without modifying the page. **Config as source of truth:** All fields (`desc`, `next_up`, `roadmap_items`) are sourced from config and regenerated on each cron run — hand-edits to the HTML output are overwritten. See `docs/solutions/integration-issues/cron-script-config-driven-content-rendering.md` for the pattern and prevention strategies.

**Feed staleness monitor** — `bin/check-feed-health.py` runs every 6 hours via `.github/workflows/feeds-staleness-check.yml`. Opens a GitHub issue (labeled `feed-stale`) when a feed's `last_success_utc` is older than 48h and auto-closes it when the feed recovers. **It does NOT comment on an already-open issue** — that branch only prints; the docs claimed otherwise until 2026-08-05. One issue is the whole signal, with no escalation. **A `last_error` on a feed whose `last_success_utc` is fresh is purely passive** — the monitor gates only on age, and `partial_success=True` refreshes the timestamp, so such a note is *structurally guaranteed* never to alert. It is visible only by reading `.feeds-heartbeat.json`. Issue body includes feed-specific actionable guidance (e.g. "run `./bin/whoop-auth.sh`" for WHOOP). GitHub emails you on issue creation via repo subscription. **Transient GitHub API errors** (5xx, gateway timeout from `gh issue list`) are caught and result in `sys.exit(0)` — the next scheduled run catches any genuinely stale feeds. Non-transient errors (e.g. 401 auth failure) still propagate and fail the workflow.

**Heartbeat-correctness invariant (added 2026-07-10):** a sync script must record a *success* heartbeat only on a path it can prove was reached with real upstream data — **never** on a no-op/"nothing changed" path that a swallowed HTTP error could also reach, or the monitor goes blind (a 403 masquerades as "healthy but quiet"). `update-spotify.py` + `update-whoop.py` set an `_api_error` flag in `api_get`'s `HTTPError` branch and bail before the heartbeat; `update-projects.py` tallies `_events_ok`/`_events_err` and bails only on the *systemic* case (zero ok, ≥1 err — a dead `TLDR_FETCH_TOKEN`). When adding a feed, verify no success-heartbeat path runs after a swallowed fetch error. Full write-up: `docs/solutions/integration-issues/feed-heartbeat-on-noop-path-hides-upstream-api-failure.md` (and `spotify-development-mode-lockdown-strands-personal-app-feed.md` for the Spotify vendor-policy root cause).
...
- `.git/hooks/pre-commit` blocks accidental commits of junk files (see blocklist in hook) **and runs the full test suite** (~3s; bypass with `SKIP_TESTS=1 git commit`). The test gate was added 2026-08-05 — this file and a solution doc had both claimed it for months while the hook was a filename blocklist only.
- `.gitignore` covers all build artifacts, OS junk, and working documents
- **`.gitignore` is version-controlled; the hook is NOT** — `.git/hooks/` is never tracked by git, so a fresh clone has no hook and the earlier "distribute via git clone" claim here was wrong. **CI is the real enforcement point**; the hook is a local fast-fail. Re-create it by hand after cloning, or point `core.hooksPath` at a tracked directory if this ever needs to be shared.
...
All code-review findings from four reviews (initial, 2026-04-18 full-repo audit, 2026-04-29 whole-repo audit, 2026-05-05 cross-repo-admin sweep) have been resolved or deferred with a documented work log. See `todos/*` for the full history — **151 files: 149 resolved (61 `complete` + 88 `done`), 2 discarded, 0 pending** (`todos/148` chore-commits-promote-projects — reviewed + deferred 2026-08-04, re-reviewed + still deferred 2026-08-05 now that the `pin` override absorbs the family-site fan-out case; `todos/149` repo-rename silent misclassification — **resolved 2026-08-05**, config fixed 8/04 plus the `repo_key_mismatch()` detector built 8/05; `todos/150` — the two untested mirrors left open by the 2026-08-05 audit — **resolved 2026-09-01**: `admin/docs` gained the disk-vs-index parity test it was the only root missing (`test_every_admin_doc_on_disk_is_indexed`, verified by stripping a real PRD's `type:` line and watching it go red), and `now/project-cards.js` was **deleted** rather than repaired — it was referenced by zero HTML and zero JS, its auto-render had been commented out for ~3 months, and its array carried a phantom `wcrn` while missing four real projects, so activating it as documented would have rendered a project that does not exist and dropped four that do. A fallback nobody renders and no test covers is a trap with instructions attached; `todos/151` — the 2026-08-16 P1 public-Actions-log disclosure, **resolved 2026-09-01**). **Filename status must match the `status:` field inside** — `todos/146` + `147` were named `*-pending-*` while carrying `status: complete`, which made `ls todos/ | grep pending` lie; renamed 2026-08-04. As of 2026-07-13 all code-review + audit items are resolved: the last open item `todos/147` (the quarterly `docs/solutions/` audit) was re-run across all 21 solution docs and its drift fixed this session. (`todos/146` — snapshot-banner asymmetry on the JT `roadmap/` + `changelog/` sub-pages — was resolved 2026-06-25 via commit `5292f04`.) The former long-pending item (`todos/129`) — admin-side CSP for the Tina shell on thirstypig.com — was resolved via Vercel HTTP headers in `vercel.json` (CSP + X-Frame-Options on `/admin/*`), which survives `tinacms build` unlike a meta tag approach. The 2026-04-29 sweep is captured in `todos/089-120`; the 2026-05-05 sweep in `todos/121-136`. Matching solution docs: `docs/solutions/integration-issues/relative-time-html-defeats-content-changed-cache.md`, `docs/solutions/integration-issues/cross-repo-admin-via-github-contents-api.md`, `docs/solutions/integration-issues/marker-boundary-content-staleness.md`, `docs/solutions/integration-issues/per-project-adapters-for-heterogeneous-roadmap-sources.md`.
...
**Health strip** (`admin/health.js` + `#health-strip` / `#health-doorbell` in `admin/index.html`) — "is anything broken or about to break?". Reads the public `.feeds-heartbeat.json` for feed freshness and `admin/status.json` for the credential-expiry doorbell (bare counts only). Gated on the `jc-admin` sessionStorage flag; `textContent`/DOM nodes only, never `innerHTML`.

**It is the READER for every detector that cannot alert (hardened 2026-09-01).** `record_heartbeat(partial_success=True)` refreshes `last_success_utc` — the very field the 48h staleness monitor gates on — so a feed dropping content on every run is *structurally incapable* of alerting. Age alone can never surface it. Two changes make the strip an honest reader: (1) **`rollUpErrors()`** attributes each `last_error` to the slug before its first colon, so `project-docs:aleph-roadmap` reaches the `project-docs` row. The sub-slugs were previously discarded under a comment claiming they "roll up into the aggregate" — they did not, and the aggregate's own `last_error` is null, so that row read green while two sub-feeds dropped content daily. (2) A fresh row carrying `last_error` renders **`note`** (`.nb-health-dot--note`, a ring not a solid fill) rather than `ok`, and rows sort by severity (`RANK`) before age. Rendering `last_error` adds no disclosure — it is already in the committed public heartbeat this page already fetches. **When adding a feed that emits a `parent:child` heartbeat slug, make sure `parent` also exists as a top-level slug**, or its errors are dropped silently; `TestAdminPortfolio::test_every_heartbeat_sub_slug_has_a_parent_row` is the guard.

**Portfolio docs hub** lives under `admin/docs/` (module-isolated with the admin feature). It's a browsable, read-only knowledge base for **all portfolio projects** — and, as of 2026-07-24, jameschang.co's own engineering docs too: `docs/guides/`, `docs/test-plan.md`, and `docs/solutions/` are indexed **in place** (never copied) under a "Site engineering" board section, project slug `jameschang-co-eng`, via per-root adapters in `bin/build-docs-index.py` (`adapt_guide` synthesizes frontmatter for the plain-markdown guides; `adapt_solution` translates the solutions' own YAML vocab). Full spec: `admin/docs/README-DOCS.md`. Key conventions:
...
- **IDs** — stable, never reused: `PRD-###` · `ADR-###` · `DOC-###` · `RISK-###` · `EXP-###`. Adapter-indexed site-engineering docs use slug IDs (`GUIDE-<stem>`, `SOL-<slug>`) derived from the source filename — never hand-assigned.
