...
(() => {
  ...
  const renderFeeds = (hb) => {
    ...
    if (!strip) return;
    // Top-level slugs only — skip colon-suffixed per-doc sub-slugs (they roll
    // up into the `project-docs` aggregate).
    const rows = Object.entries(hb)
      .filter(([slug]) => !slug.includes(":"))
      .map(([slug, info]) => ({ slug, h: ageHours(info && info.last_success_utc) }))
      .sort((a, b) => b.h - a.h); // worst (stalest) first
    strip.replaceChildren(...rows.map(({ slug, h }) => {
      const row = el("div", "nb-health-row");
      row.append(el("span", `nb-health-dot nb-health-dot--${rag(h)}`));
