...
6. Call `record_heartbeat("feed_name")` from `_shared.py` in the sync script's `main()` — **only on a path you can prove was reached with real upstream data.** Set an error flag in the fetch's exception branch and bail *before* the heartbeat; see `_api_error` in `update-spotify.py` / `update-whoop.py`. **Do not record a heartbeat on the early-return "nothing changed" path** — a swallowed HTTP error reaches that path too, so the monitor goes blind and a 403 reads as "healthy but quiet". This guide previously said to heartbeat on both paths, which is the exact bug written up in `docs/solutions/integration-issues/feed-heartbeat-on-noop-path-hides-upstream-api-failure.md` (it cost weeks of a silently dead Spotify feed). The 6-hour staleness check (`bin/check-feed-health.py` run by `.github/workflows/feeds-staleness-check.yml`) opens a GitHub issue (labeled `feed-stale`) if any feed's `last_success_utc` is >48h old and auto-closes it when the feed recovers. **It does not comment on an already-open issue** — that branch only logs, so one issue is the entire signal; there is no escalation. Note: the 48h threshold may not suit feeds that update less frequently (e.g. weekly).
