...
**External (the trigger):** Spotify's data endpoints returned **HTTP 403** (not 401). Token refresh (`get_access_token()`) still succeeded — no "Token refresh failed" line — so the refresh token was alive. A 403-not-401 on user-data endpoints means *authenticated but not authorized*. **Follow-up (2026-07-11) found the real cause was NOT a revoked scope but Spotify's Feb 2026 Development Mode lockdown — re-auth does not reliably fix it. See [`spotify-development-mode-lockdown-strands-personal-app-feed.md`](./spotify-development-mode-lockdown-strands-personal-app-feed.md).** (Original guessed fix, kept for the record: re-run `./bin/spotify-auth.sh` to mint a fresh grant with the correct scopes — the script requests `user-read-recently-played user-read-playback-state` — and update the `SPOTIFY_REFRESH_TOKEN` secret; if it persists, check spotify.com/account/apps and the Developer Dashboard app state.)
...
- **`update-whoop.py`**: a data-endpoint 403 (WHOOP's exact analog to Spotify) is swallowed to `{"records": []}`, `build_html(None, None, None)` renders em-dash placeholders, and both heartbeat calls (`:301`, `:306`) fire with no `error=`. Same silent-outage risk.
- **`update-projects.py`**: `fetch_repo_events` swallows `HTTPError`/`URLError` to `[]` (`:156`). A persistent 403 — e.g. an **expired `TLDR_FETCH_TOKEN`** — makes every project render "no recent activity" while `record_heartbeat("projects")` (`:585`/`:590`) keeps `last_success_utc` fresh. Ironically, `check-feed-health.py`'s own guidance for the `projects` slug blames an expired `TLDR_FETCH_TOKEN` — the very failure this script currently can't surface.
...
- [`hand-listed-ci-test-files-silently-exclude-new-tests.md`](./hand-listed-ci-test-files-silently-exclude-new-tests.md) — the same shape one layer up: a green CI check recorded on a path that proved nothing, because the workflow named two test files and the suite had grown to twelve. The regression tests written for *this* outage were among the 368 that never ran.
- [`github-repo-rename-redirect-silently-orphans-project-events.md`](./github-repo-rename-redirect-silently-orphans-project-events.md) — the successor case, one level deeper: the fetch genuinely **succeeded** (HTTP 200 via GitHub's rename redirect) and the result was misfiled, so the `_events_ok == 0 and _events_err > 0` gate added here cannot see it. Error-handling guards catch failed fetches, not misattributed ones. **Update 2026-08-05:** a *content-level* guard was built for exactly that case (`repo_key_mismatch()`), and it introduced a third heartbeat state this doc's binary record/skip framing does not cover — `partial_success=True`, which refreshes `last_success_utc` **and** records `last_error`, so a permanent-but-non-fatal defect stays visible without false-tripping the 48h monitor.
