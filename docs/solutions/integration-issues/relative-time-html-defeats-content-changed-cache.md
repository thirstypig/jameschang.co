...
2. **Companion strip.** `strip_volatile()`'s docstring explicitly names what it strips. Any new emitter pattern (e.g., a "released N days ago" badge with a different element shape) needs a matching regex added to `strip_volatile()` in the same change. The no-op assertion in `tests/test_shared.py::TestContentChanged` is what enforces this against new fixture content. **Correction (2026-08-05):** this line previously credited the pre-commit hook, which until that date was a filename blocklist that never invoked pytest — and CI ran a hand-listed two-file subset that excluded most of the suite, so the invariant had *neither* documented enforcement point actually running. Both are now real: CI globs `tests/`, and the hook runs the suite locally. See [`hand-listed-ci-test-files-silently-exclude-new-tests.md`](./hand-listed-ci-test-files-silently-exclude-new-tests.md).
