...
> **Status note (2026-08-05): this upgrade path has NOT shipped.** A config flag named `pin` now exists and *is* read by `update-projects.py` — but it is a different axis. `pin` began as section membership only (active vs back-burner), and was extended the same day into ordering *within* back-burner via `pin_backburner_last()` — but only for pinned projects, and it sinks them to the bottom. The proposal above concerns ordering for **self-referential** projects, which is still `SELF_SLUG` + `pin_self_last()`, still hardcoded and untouched (and still sequenced last, so it wins the final slot). Do not read the existence of `pin` as this item being done. See [`activity-recency-misreads-maintenance-mode-as-active-work.md`](./activity-recency-misreads-maintenance-mode-as-active-work.md).
