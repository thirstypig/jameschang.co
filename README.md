...
| `now/index.html` | Derek Sivers-style /now page with 8 cron-synced data feeds (incl. Google Calendar) + 12 cron-classified per-project TLDR/shipping cards + client-rendered sections (hitlist, bucket list, quotes) |
...
- 555 tests (pytest, across 13 files). CI runs the whole `tests/` directory on every push to `main`; a local pre-commit hook runs the same suite (~3s, `SKIP_TESTS=1` to bypass). The hook lives in `.git/hooks/` and is therefore **not** version-controlled — CI is the real enforcement point.
